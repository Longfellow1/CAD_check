...
from .domain import CheckExecution, RegressionResult
...
class ArtifactStore:
    ...
    def write_run(self, run_id: str, baseline: list[CheckExecution], candidate: list[CheckExecution], regression: list[RegressionResult]) -> Path:
        ...
        with (path / "trace.jsonl").open("w", encoding="utf-8") as f:
            for run_kind, executions in (("baseline", baseline), ("candidate", candidate)):
