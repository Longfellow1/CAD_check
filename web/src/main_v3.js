...
const JOB_PHASE = {
  QUEUED:'等待 Worker', STARTING:'Worker 已接收', READING:'读取模型',
  INVENTORY:'构建装配树', TESSELLATING:'生成三维', CHECKING:'执行校核',
  FINALIZING:'固化证据', DONE:'完成', FAILED:'失败', CANCELLED:'已取消',
  WORKER_RESTARTING:'重启 Worker', WORKER_CRASHED:'Worker 异常',
};
const state = {
  models:{}, cards:[], model:null, openInfo:null, assembly:null, tab:TAB.CHECKS,
  viewer:null, activeJob:null, selectedCase:null, selectedOccurrence:null,
  quickMeasure:false, sectionEnabled:false, checkScope:'SET', checkCardId:null,
  runs:{check:null, regression:null, explore:null}, evidenceModel:null,
  autoShots:new Set(), runtimeState:null, workerState:null,
};
...
<div class="shell">
  ...
  <main class="workspace">
    ...
    <section class="panel viewer-panel">
      ...
      <div id="viewer-badge" class="viewer-badge">BABYLON · OCP DERIVATIVE</div>
    </section>
...
function setProgress(text, {indeterminate=false, done=false, cancellable=false}={}) {
  const bar = $('#progress');
  bar.classList.toggle('indeterminate', indeterminate);
  bar.style.width = done ? '100%' : (indeterminate ? '30%' : '0%');
  $('#progress-text').textContent = text;
  $('#cancel').classList.toggle('hidden', !cancellable);
}
function updateJobStatus(job) {
  const label = JOB_PHASE[job.progress_phase] || job.message || job.state;
  const terminal = ['SUCCEEDED','FAILED','CANCELLED','TIMED_OUT'].includes(job.state);
  setProgress(label, {indeterminate:!terminal, done:job.state === 'SUCCEEDED', cancellable:!terminal});
  setStatus('#status-worker', `Worker: ${job.worker_pid ? `PID ${job.worker_pid}` : job.state}`, job.state === 'FAILED' || job.state === 'TIMED_OUT' ? 'error' : 'ok');
}
...
function ensureViewer() {
  ...
  state.viewer = new BabylonCadViewer($('#cad'), {
    onPick: (occurrenceId) => selectOccurrence(occurrenceId, {loadDetail:false}),
  });
...
function leafNodes() {
  return flattenTree(state.assembly?.roots || [], []).filter((node) => !node.children?.length);
}
function nodeById(id) {
  return flattenTree(state.assembly?.roots || [], []).find((node) => node.occurrence_id === id) || null;
}

function acceptOpenedModel(result) {
  ...
  $('#viewer-meta').textContent = `${stats.leaf_count} leaves · proxy-first`;
  $('#readiness-out').textContent = JSON.stringify({
    preview:'READY',
    engineering_check: result.version === 'V1' || result.version === 'V2' ? 'CONTROLLED_BINDINGS_AVAILABLE' : 'REQUIRES_CASE_BINDING',
...
async function loadModel(key=state.model) {
  if (!key) return;
  state.model = key;
  $('#model').value = key;
  ...
  // Small controlled fixtures can be made visually complete immediately.  A
  // Scania-size model deliberately stays proxy-first so Renderer memory does
  // not grow with every leaf; detail is requested only for engineering focus.
  if (result.leaf_count <= 80) {
    const paths = leafNodes().filter((node) => node.valid).map((node) => node.original_path);
    if (paths.length) {
      try {
        const detail = await submitJob('DETAIL', {model:key, paths, deviation:0.35, angular_tolerance:0.35, render_edges:false}, 240);
        state.viewer.loadDetail(detail);
        $('#viewer-meta').textContent = `${result.leaf_count} leaves · detail resident`;
        setStatus('#status-viewer', 'Viewer: Babylon detail', 'ok');
      } catch (error) {
        console.warn('Small model detail load failed', error);
      }
    }
  } else {
    setProgress('Overview 可交互', {done:true});
  }
}

async function openNativeStep() {
  if (!window.cadDesktop?.isDesktop || !window.cadDesktop?.pickStepFile) {
    throw new Error('独立浏览器仅为调试 Harness；产品导入必须从 Electron 原生文件选择器发起。');
  }
  const path = await window.cadDesktop.pickStepFile();
  if (!path) return;
  const response = await api('/api/desktop/models/register', {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({path})});
...
function treeNodeHtml(node, depth=0) {
  ...
  const childHtml = children.length ? `<div class="tree-children">${children.map((child) => treeNodeHtml(child, depth + 1)).join('')}</div>` : '';
  return `<div class="tree-node"><button class="tree-row ${node.valid ? '' : 'invalid'} ${state.selectedOccurrence?.occurrence_id === node.occurrence_id ? 'active' : ''}" data-occ="${escapeHtml(node.occurrence_id)}" style="--depth:${depth}"><span class="tree-caret">${children.length ? '▾' : '·'}</span><span class="tree-name">${escapeHtml(node.name)}</span></button>${childHtml}</div>`;
}
function renderModelTree() {
  ...
  document.querySelectorAll('.tree-row').forEach((element) => element.onclick = () => selectOccurrence(element.dataset.occ, {loadDetail:false}));
}
...
function checkRows() {
  ...
  return executions.map((execution) => ({caseId:execution.case_id,title:execution.title,outcome:execution.status,execution,run,kind:'check'})).sort((a,b) => priority(a.outcome)-priority(b.outcome));
}
function regressionRows() {
  const run = state.runs.regression;
  if (!run) return [];
  return (run.regression || []).map((regression) => ({
    caseId:regression.case_id,
    title:regression.title || regression.case_id,
    outcome:regression.regression,
    execution:regression.candidate,
    regression,
    run,
    kind:'regression',
...
function resultRowHtml(row) {
  ...
  const authority = execution.rule_authority ? `<span class="authority">${escapeHtml(LABELS[execution.rule_authority] || execution.rule_authority)}</span>` : '';
  return `<button class="result-row ${state.selectedCase === row.caseId ? 'active' : ''}" data-case="${escapeHtml(row.caseId)}" data-kind="${row.kind}"><div class="result-main"><span class="result-status ${statusClass(row.outcome)}">${escapeHtml(LABELS[row.outcome] || row.outcome)}</span>${authority}</div><b>${escapeHtml(row.title)}</b><div class="result-meta"><span>${execution.value ?? '—'} ${escapeHtml(execution.unit || '')}</span><span>${escapeHtml(row.caseId)}</span></div></button>`;
}
function bindResultRows() {
  document.querySelectorAll('.result-row').forEach((element) => element.onclick = () => selectResult(element.dataset.case, element.dataset.kind));
}
function renderChecks() {
  const rows = checkRows();
  $('#nav-content').innerHTML = rows.length ? `<div class="result-list">${rows.map(resultRowHtml).join('')}</div>` : '<div class="empty-stack"><b>尚未运行校核</b><span>默认执行 MVP 18 条 Check Set。</span></div>';
  bindResultRows();
}
function renderRegression() {
  const rows = regressionRows();
  $('#nav-content').innerHTML = rows.length ? `<div class="regression-head"><span>Baseline V1</span><span>→</span><span>Candidate V2</span></div><div class="result-list">${rows.map(resultRowHtml).join('')}</div>` : '<div class="empty-stack"><b>尚未运行版本回归</b><span>同一 FORMAL Check Set 比较 V1/V2。</span></div>';
  bindResultRows();
}
...
async function loadOccurrenceDetail(node=state.selectedOccurrence) {
  ...
  try {
    const detail = await submitJob('DETAIL', {model:state.model, paths:[node.original_path], deviation:0.2, angular_tolerance:0.25, render_edges:true}, 180);
    state.viewer.loadDetail(detail);
    ...
    renderInspectorMessage('局部几何加载失败', `${error.message}。Overview 与 Canonical Tree 仍可继续使用。`, 'warn');
  }
}
...
function renderResultInspector(row) {
  const execution = row.execution || {};
  const status = row.outcome || execution.status;
  ...
  $('#inspector').innerHTML = `<div class="metric">${execution.value ?? '—'} <small>${escapeHtml(execution.unit || '')}</small></div>${compare}<dl class="kv"><dt>Case</dt><dd>${escapeHtml(row.caseId)}</dd><dt>成熟度</dt><dd>${escapeHtml(LABELS[execution.rule_authority] || execution.rule_authority || '—')}</dd><dt>Executor</dt><dd>${escapeHtml(execution.executor || '—')}</dd><dt>阈值</dt><dd>${escapeHtml(thresholdText(row))}</dd><dt>Margin</dt><dd>${execution.margin ?? '—'}</dd><dt>Method</dt><dd>${escapeHtml(execution.evidence?.measurement_method || '—')}</dd><dt>Evidence</dt><dd>${escapeHtml(execution.evidence?.annotation || '—')}</dd></dl><div class="action-stack"><button id="view-evidence" class="primary">重新定位 Evidence</button><button id="replay-evidence" class="ghost">Replay 已保存状态</button>${row.kind === 'regression' ? '<div class="segmented"><button id="view-v1">看 V1</button><button id="view-v2" class="active">看 V2</button></div>' : ''}</div>${regression?.non_comparable_reason ? `<p class="panel-message warn">${escapeHtml(regression.non_comparable_reason)}</p>` : ''}<p class="hint">正式数值来自 Python/OCP B-Rep；Babylon 只显示和复核。</p>`;
  $('#view-evidence').onclick = () => showEvidence(row, row.kind === 'regression' ? (state.evidenceModel || 'V2') : state.model, {persist:true});
  $('#replay-evidence').onclick = () => replayEvidence(row);
  if ($('#view-v1')) $('#view-v1').onclick = () => showEvidence(row, 'V1', {persist:false});
  if ($('#view-v2')) $('#view-v2').onclick = () => showEvidence(row, 'V2', {persist:false});
}
function renderOccurrenceInspector() {
  ...
  $('#inspector').innerHTML = `<h3 class="object-title">${escapeHtml(node.name)}</h3><dl class="kv"><dt>Occurrence</dt><dd class="mono">${escapeHtml(node.occurrence_id)}</dd><dt>Path</dt><dd class="mono">${escapeHtml(node.original_path)}</dd><dt>Children</dt><dd>${node.children?.length || 0}</dd></dl><div class="action-stack">${!node.children?.length ? '<button id="load-detail" class="primary">加载局部精细几何</button>' : ''}<div class="segmented"><button id="isolate-node">隔离</button><button id="hide-node">隐藏</button></div></div><p class="hint">树和 occurrence identity 来自 XCAF，不依赖 Viewer Mesh。</p>`;
  if ($('#load-detail')) $('#load-detail').onclick = () => loadOccurrenceDetail(node);
...
function renderCheckConfig() {
  ...
  $('#inspector').innerHTML = `<label class="field-label">执行范围</label><select id="check-scope" class="field-control"><option value="SET" ${state.checkScope === 'SET' ? 'selected' : ''}>MVP Check Set · 18 Cases</option><option value="SINGLE" ${state.checkScope === 'SINGLE' ? 'selected' : ''}>单 Case</option></select><div id="single-card" class="${state.checkScope === 'SINGLE' ? '' : 'hidden'}"><label class="field-label">Check Card</label><select id="check-card" class="field-control">${options}</select></div><div class="card-summary"><b>3 Executors</b><p class="hint">Minimum Clearance / Directional Distance / Angle。Check Set 验证复用，不允许 18 套定制代码。</p></div>`;
...
async function selectResult(caseId, kind) {
  ...
  state.quickMeasure = false;
  $('#quick-measure').classList.remove('active');
  if (kind === 'regression') state.tab = TAB.REGRESSION;
  else state.tab = TAB.CHECKS;
  renderNavigation();
  renderInspector();
  const row = currentSelectedRow();
  if (!row || ['BLOCKED','NON_COMPARABLE'].includes(row.outcome)) return;
  const model = row.kind === 'regression' ? 'V2' : state.model;
  await showEvidence(row, model, {persist:['FAIL','REVIEW_REQUIRED','NEW_FAIL','REGRESSED'].includes(row.outcome)});
}
...
async function runEngineeringCheck() {
  ...
  renderNavigation();
  renderInspector();
  if (first && !['BLOCKED'].includes(first.outcome)) await selectResult(first.caseId, 'check');
}
...
async function runQuickMeasure() {
  ...
  if (target === counterpart) return renderInspectorMessage('快速测量', '对象 A 与对象 B 不能相同。', 'warn');
  const bindings = {bindings:{target:{default:target}, counterpart:{default:counterpart}}};
  const result = await submitJob('EXPLORE', {model:state.model,target:'target',counterpart:'counterpart',executor:'minimum_clearance',bindings,title:'快速测量'}, 300);
...
async function showEvidence(row, model, {persist=false}={}) {
  if (!row?.run?.run_id || !model) return;
  const result = await submitJob('EVIDENCE_DETAIL', {model, run_id:row.run.run_id, case_id:row.caseId}, 300);
  state.evidenceModel = model;
  const execution = result.execution || row.execution;
  const detail = result.detail;
  const viewer = ensureViewer();
  viewer.loadDetail(detail, {replace:true});
  ...
  $('#viewer-meta').textContent = 'Evidence Detail · Babylon';
  if (persist && ['FAIL','REVIEW_REQUIRED'].includes(execution.status)) await persistEvidence(row, execution);
...
async function persistEvidence(row, execution) {
  ...
  await Promise.all([
    api(`/api/runs/${row.run.run_id}/evidence/${encodeURIComponent(row.caseId)}/screenshot`, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({data_url})}),
    api(`/api/runs/${row.run.run_id}/evidence/${encodeURIComponent(row.caseId)}/view-state`, {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({state:viewState})}),
  ]);
...
async function replayEvidence(row) {
  ...
  if (state.model !== modelKey || !state.assembly) await loadModel(modelKey);
  const available = new Set(flattenTree(state.assembly?.roots || [], []).map((node) => node.occurrence_id));
  const required = saved.focus_occurrence_ids || [];
  if (required.some((id) => !available.has(id))) return showReplayFallback(record, 'Occurrence Identity 不兼容，已降级为记录/截图查看。');
...
async function checkRuntimeAndWorker() {
  ...
  if (desktop?.isDesktop) {
    ...
    $('#product-form').textContent = 'DEBUG HARNESS：当前不是 Electron 产品环境，不能作为 MVP 验收。';
  ...
  try {
    ...
    setStatus('#status-worker', `Worker: ${worker.state}${worker.pid ? ` · ${worker.pid}` : ''}`, worker.state === 'RUNNING' ? 'ok' : 'warn');
    $('#runtime-detail').textContent = JSON.stringify({runtime:state.runtimeState,worker}, null, 2);
  } catch (error) {
    setStatus('#status-worker', 'Worker: unavailable', 'error');
...
async function restartRuntime() {
  ...
  await window.cadDesktop.restartRuntime();
}
...
$('#load').onclick = () => loadModel().catch((error) => renderInspectorMessage('模型加载失败', error.message, 'error'));
$('#open-step').onclick = () => openNativeStep().catch((error) => renderInspectorMessage('STEP 打开失败', error.message, 'error'));
$('#cancel').onclick = () => cancelActiveJob().catch(() => {});
$('#run').onclick = () => runPrimary().catch((error) => renderInspectorMessage('执行失败', error.message, 'error'));
...
$('#restart-worker').onclick = () => restartWorker().catch((error) => renderInspectorMessage('Worker 重启失败', error.message, 'error'));
$('#restart-runtime').onclick = () => restartRuntime().catch((error) => renderInspectorMessage('Runtime 重启失败', error.message, 'error'));
$('#trace-btn').onclick = () => showTrace().catch((error) => { $('#trace-out').textContent = error.message; });
...
(async () => {
  try {
    ...
    state.checkCardId = state.cards[0]?.id || null;
    await refreshModels();
    renderNavigation();
    renderInspector();
